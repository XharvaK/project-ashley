...
| Canonical Architecture Roadmap | [`docs/architecture/Ashley_Architecture_Roadmap.md`](Ashley_Architecture_Roadmap.md) | Current implementation boundary, owner-selected delivery map, classified dependency edges, phase names, delivery focus, engineering milestones from live state, and framework disposition | 2026-08-21; milestone conversion 2026-08-23; metacognition policy 2026-08-25 | `AUTHORITATIVE` | Canonical source for current architectural direction beneath governance. The 2026-08-23 conversion adds no phases. The 2026-08-25 metacognition policy is an accepted named profile across the cognitive track. It does not add a standalone phase, freeze-map owner, or §5 milestone. |
...
| Title | Path | Topic | Era / date | Status | Current relevance |
|---|---|---|---|---|---|
| Routing Status | [`docs/Routing_Status.md`](../Routing_Status.md) | Purpose routes, providers, models, quotas, disabled routes | Wave 1; living source status; route-table audit 2026-08-25 | `SUPPORTING` | Living source snapshot. Route facts are split across `config/models.json`, `PURPOSE_TO_ROUTE`, and registry dispatch. Read that file for current model IDs. Not Model Fabric architecture. |
| Model Fabric Architecture | [`docs/architecture/Model_Fabric_Architecture.md`](Model_Fabric_Architecture.md) | Semantic dispatch boundary, specialist portfolio, MF-M1 seam, OpenCode Track A/B split | 2026-08-25 | `CURRENT PHASE CONTRACT` | **Sole** current-facing Model Fabric owner. Selected first **code** milestone is **MF-M1**. Owner scope is closed. Runtime is `PENDING` on canonical runtime integration baseline `e36613b…`. Historical MF docs checkpoint `7a788375…` and pre-repair planning baseline `8eedad8…` remain provenance. Historical F1 Thought-observation is **F1-obs** (deferred). |
...
| Model Fabric owner decision packet | [`docs/handoffs/MODEL_FABRIC_OWNER_DECISION_PACKET.md`](../handoffs/MODEL_FABRIC_OWNER_DECISION_PACKET.md) | Closed owner + ChatGPT Fabric decisions | 2026-08-25 | `SUPPORTING` | Decision record. Not a second architecture. MF-M1 is first code milestone; this packet does not implement it. Current integration baseline is `e36613b…`; the `7a788375…` checkpoint and `8eedad8…` line are historical provenance. |
| Model Fabric roadmap handoff | [`docs/handoffs/MODEL_FABRIC_ROADMAP_HANDOFF.md`](../handoffs/MODEL_FABRIC_ROADMAP_HANDOFF.md) | MF-M1 initiation handoff | 2026-08-25 | `SUPPORTING` | Docs freeze. Runtime implementation `PENDING` on `e36613b…`. Not architecture. |
| Model Fabric MF-M1 implementation checkpoint | [`docs/handoffs/MODEL_FABRIC_MF_M1_IMPLEMENTATION_CHECKPOINT.md`](../handoffs/MODEL_FABRIC_MF_M1_IMPLEMENTATION_CHECKPOINT.md) | Resume gate, file map, characterization/falsification plan | 2026-08-25 | `SUPPORTING` | Continuation point on the post-OF exact integration SHA `e36613b…`. Does not implement MF-M1. |
...
| Learned Autonomy Architecture | [`docs/architecture/Learned_Autonomy_Architecture.md`](Learned_Autonomy_Architecture.md) | Evidence-bound interests, preferences, salience, and goals | 2026-08-21 | `CURRENT PHASE CONTRACT` | Advances Ashley herself. Learned preference is not authority. Does not depend on Computer Use. Inherited/current-interest separation is C3-closing, not C1-closing. |
| Context Budget Architecture | [`docs/architecture/Context_Budget_Architecture.md`](Context_Budget_Architecture.md) | Bounded attention over persistent state | 2026-08-21 | `CURRENT PHASE CONTRACT` | Context eviction is not forgetting. Minimal Model Fabric `ContextProjection` is transport only. I2/I3 remote egress requires explicit, scope-bound approval and an approved route. |
| Cognitive Graduation Architecture | [`docs/architecture/Cognitive_Graduation_Architecture.md`](Cognitive_Graduation_Architecture.md) | Epistemic maturation, lived-experience continuity, integrated cognitive coherence | 2026-08-21 | `CURRENT PHASE CONTRACT` | Integration and qualification over existing cognitive owners. Not a catch-all and not Relational Graduation. Selected prediction/outcome calibration is consumed here. |
| Relational Graduation Architecture | [`docs/architecture/Relational_Graduation_Architecture.md`](Relational_Graduation_Architecture.md) | Mutual commitment, tension, withdrawal, non-manipulation, companion continuity | 2026-08-21 | `CURRENT PHASE CONTRACT` | Sibling of Cognitive Graduation. Independent state, evidence, rollback, and acceptance. Shared-culture recomputation is C5-closing, not C1-closing. |
...
| `Ashley_Glossary.md` omits Mind State, Thought, Reflection | Freeze and Cross-Phase list those as cognitive owners. | Glossary lag. Do not infer absence of the owners. Repair is separate from this metacognition planning pass. |
...
| Architecture Freeze: “Ashley does not need anything new” as a cognitive faculty | 2026-08-25 metacognition planning names a *cross-cutting policy profile* with mechanisms owned elsewhere. | Pass-3 accepted: not a freeze-map owner. Do not treat `Ashley_Metacognition_Architecture.md` as a freeze-map amendment. A later Freeze amendment would require an independently owned lifecycle that existing owners cannot express. |
| Roadmap §5.6 Sandbox `CURRENT WORK` vs §3 Model Fabric as owner-selected current delivery | Phase-contract register vs live delivery sentence. | Treat §3 + Model Fabric docs as live delivery. MF-M1 runtime remains `PENDING`. §5.6 is not a dashboard. Pass-3 did not repair this wording. |
