...
| `G_MODEL_SEMANTIC_NONCOMPLIANCE` | `PROVEN` | Under the frozen W2 expected-kind contract, all three settlement prompts received valid abstain objects. JSON, closed schema, strict parser, Kernel binding, and request identity passed; semantic validity failed with `semantic_branch_mismatch`. | `HIGH` for the current 4 rejected rows; this is not proof of fundamental incapability. |
