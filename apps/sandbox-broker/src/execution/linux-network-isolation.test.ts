...
} = {}): { provider: NetworkIsolationProvider; runner: RecordingRunner } {
...
describe("LinuxUnshareNetworkIsolation", () => {
  ...
  it("7. missing sysctl knobs are unknown and do not block the probe", async () => {
    ...
    expect(runner.calls).toHaveLength(0); // prepare alone never spawns
  });
  ...
  it("11. status is operational only when every prerequisite holds", async () => {
    ...
    expect(makeProvider().provider.status()).toBe("operational");
  });
