...
'if [[ "$2" == "crypto-types" ]]; then rm "$RUNTIME_ROOT/crypto/types.js"; fi',
          'printf \'%s\\n\' \' "use strict"; export const ready = true; export function validate(x) { return x; };\' > "$RUNTIME_ROOT/execution/qualification-toolchain.js"',
          'if [[ "$1" == "none" ]]; then',
          '  printf \'%s\\n\' \'process.on("uncaughtException", (err) => { process.exit(1); });\' > /dev/null',
          'fi',
          'sudo() {',
          '  while (($# > 0)); do',
          '    case "$1" in',
