...
describe("v0.2.1 lexical discovery fallback", () => {
  ...
  it("discovers live and quarantined Memory from current trigger terms", () => {
    ...
    try {
      ...
      const relevant = retrieveCandidates(db, { conversationId: "thread-1", request: { triggerTerms: ["qwen"], workingContextTopics: [], assertionKeys: [], includeLogSearch: true } });
      ...
      const unrelated = retrieveCandidates(db, { conversationId: "thread-1", request: { triggerTerms: ["unrelated"], workingContextTopics: [], assertionKeys: [], includeLogSearch: true } });
