...
/**
 * Mistral provider adapter. The SDK call and response normalization live
 * here so `mistral-client.completeChat` only owns attention plumbing.
 */
export function createMistralAdapter(
  ...
  return {
    ...
    async dispatch({ messages, modelId, options, signal }) {
      ...
      const text = typeof raw === "string" ? raw : extractTextDelta(raw);
      const rawToolCalls =
