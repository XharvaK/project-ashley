...
describe("C3 additive schema", () => {
  it("creates typed influence, evidence, receipt, and seed-lineage tables", () => {
    ...
    try {
      expect(NUCLEAR_SUPPORTED_VERSION).toBe(38);
      expect(db.prepare("PRAGMA user_version").get()).toEqual({ user_version: 38 });
  ...
  it("rejects a C3 object when a v37 reader validates newer content", () => {
    const db = openNuclearDb(new DatabaseSync(":memory:"));
    try {
      db.exec("PRAGMA user_version = 37");
      expect(() => validateNuclearSchemaContent(db, 37, { rejectNewerContent: true }))
        .toThrow(/unexpected_v38/);
    } finally {
      db.close();
    }
  });
